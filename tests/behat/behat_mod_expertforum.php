 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_mod_expertforum extends behat_base {
     ...
     *
     * @Given /^I add a new question to "(?P<forum_name_string>(?:[^"]|\\")*)" expertforum with:$/
     * @param string $forumname
     * @param TableNode $table
     */
    public function i_add_a_new_question_to_expertforum_with($forumname, TableNode $table) {
        return $this->add_new_post($forumname, $table);
    }
     ...
     * @return Given[]
     */
    protected function add_new_post($forumname, TableNode $table) {

        $buttonstr = "Ask question"; // TODO string.
        $savechanges = "Save changes"; // TODO string.
        return array(
            new Given('I follow "' . $this->escape($forumname) . '"'),
